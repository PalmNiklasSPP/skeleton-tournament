...
export function popup(triggerNode: HTMLElement, args: PopupSettings) {
  ...
  function setDomElements(): void {
    ...
    elemArrow =
      elemPopup.querySelector(`.arrow`) ?? document.createElement("div");
  }
  setDomElements(); // init
  ...
  function open(): void {
    ...
    focusablePopupElements = Array.from(
      elemPopup?.querySelectorAll(focusableAllowedList)
    );
  }
  ...
  switch (args.event) {
    ...
    case "hover":
      let id;
      triggerNode.addEventListener(
        "mouseover",
        () => (id = setTimeout(open, 500)),
        true
      );
      triggerNode.addEventListener(
        "mouseleave",
