<script lang="ts">
  ...
  import { popup } from "$lib/utilities/palm-popup";
		...
		<tbody class="table-body {regionBody}">
			{#each source.body as row, rowIndex}
				...
				<tr
					...
					use:popup={  {  event: "hover",
					target: "popupHover-"+rowIndex,
					placement: "top"
					}}
				...
				<div class="card p-4 w-72 shadow-xl" data-popup="popupHover-{rowIndex}">
					<div class="space-y-4">
					  <!-- <Avatar src="src\lib\grid_0.png" width="w-16" /> -->
					  <div>
						<div class="flex">
						  <p class="font-bold">Niklas Palm</p>
						  <p class="ml-1">#1</p>
						</div>
						<p class="opacity-50">@NickeP</p>
					  </div>
					  <p>
						Skilled and strategic navigates the pool table with precision, turning
						complex shots into art and making the cue ball dance to their tune.
					  </p>
					  <div class="flex gap-4">
						<small><strong>7</strong> <span class="opacity-50">Wins</span></small>
						<small><strong>2</strong> <span class="opacity-50">Losses</span></small>
						<small><strong>9</strong> <span class="opacity-50">Played</span></small>
					  </div>
					</div>
